			...
			void *(*start_routine) (void *), void *arg)
{
	...
	// only execute upon the first call of lpthread_create()
	static char first = 0;
	if(first == 0){
		// initialize all lpthread_t statuses to untouched
		int i;
		for(i = 0 ; i < 511 ; i++){
	...
	table[tid].lcontext.uc_link = 0;		

	// set up stack

	table[tid].lcontext.uc_stack.ss_sp = malloc(MEM);
	table[tid].lcontext.uc_stack.ss_size = MEM;
	table[tid].lcontext.uc_stack.ss_flags = 0;


	// makecontext

	if(arg == NULL){
		makecontext( &(table[tid].lcontext) , (func) start_routine, 0);
	}else{
		makecontext( &(table[tid].lcontext) , (func) start_routine, 1, arg);
	}
...
int lpthread_yield(void){
	...
	swapcontext(&(table[prev].lcontext),&(table[currActive].lcontext));
